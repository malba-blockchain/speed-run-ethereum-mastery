...
const Streamer: NextPage = () => {
  ...
  function recieveVoucher(clientAddress: string) {
    ...
    async function processVoucher({ data }: { data: Pick<Voucher, "signature"> & { updatedBalance: string } }) {
       ...
       *
       *  recreate the  hashed, and arpacked,rayified message from reimburseService (above),
      ...
      // Encode the 'updatedBalance' as a packed uint256 value
      ...
      // Convert the hashed result into an array of bytes
      ...
      if(valid) {
        ...
        if (existingVoucher === undefined || updatedBalance < existingVoucher.updatedBalance) {
          ...
          console.log("The verification of the message is working");
        }

      }
      else {
        console.warn('The voucher is not valid, there was an error in verifyMessage()');
        return;
      }
    }
  }
...
//You'll want to compare the size of your best voucher against the size of your provided wisdom. 
//If there's too big a discrepency, cut them off
  const provideService = (client: AddressType, wisdom: string, biggestVoucher: string) => {

    var voucherSize = parseFloat(biggestVoucher)*100;
    console.log("This is the size of the best voucher: ", voucherSize);
    console.log("This is the size of the provided wisdom voucher: ",  wisdom.length);

    if (wisdom.length> voucherSize+5) {
      var payForWisdom = "Pay to get more wisdom";
      setWisdoms({ ...wisdoms, [client]: payForWisdom });
      channels[client].postMessage(payForWisdom);
    }
    else {
                  ...
                  <div key={clientAddress} className="w-full flex flex-col border-primary border-t py-6">
                    ...
                    <textarea
                      ...
                      onChange={e => {
                        ...
                        /*
                        console.log("This is the size of the best voucher: ", vouchers[clientAddress]
                        ? formatEther(parseEther(STREAM_ETH_VALUE) - vouchers[clientAddress].updatedBalance)
                        : 0);
                        */
                        var bestVoucher = vouchers[clientAddress]
                        ? formatEther(parseEther(STREAM_ETH_VALUE) - vouchers[clientAddress].updatedBalance)
                        : "0";
                        provideService(clientAddress, updatedWisdom, bestVoucher);
                      }}
